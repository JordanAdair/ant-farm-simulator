...
export class SimulationEngine {
  ...
  public update() {
    ...
    // 2. Spawn surface food periodically via fruit drops
    this.frameCount += mult;
    if (this.frameCount >= CONFIG.FOOD_SPAWN_INTERVAL * 60) {
      this.triggerFruitDrop();
      this.frameCount = 0;
    }

    // Progress fruit growth & update falling physics
    this.trees.forEach(tree => {
      tree.fruits.forEach(fruit => {
        if (fruit.isFalling) {
          fruit.vy += 0.15 * mult; // gravity
          fruit.x += fruit.vx * mult;
          fruit.y += fruit.vy * mult;
          ...
          if (this.grid.isValid(col, row)) {
            const cell = this.grid.cells[col][row];
          ...
          if (collided) {
            ...
            const surfaceR = this.grid.getSurfaceRow(foodCol);
            
            this.grid.spawnFoodAt(foodCol, surfaceR - 1, 350);
            this.colony.addLog('A massive fallen apple bursts open on the ground, providing a huge stockpile of food.', 'system');
          ...
          // Grow hanging fruit
          if (fruit.growth < 100) {
            fruit.growth = Math.min(100, fruit.growth + 0.001 * mult);
          }
        }
      });
    });
  ...
  public render() {
    ...
    const baseWind = this.weather === 'Rainy' ? -0.45 : 0.05;
    const windAmplitude = this.weather === 'Rainy' ? 0.35 : 0.12;
    const sway = baseWind + Math.sin(windTime) * windAmplitude;

    // 1. Draw Grass Blades
    this.grassBlades.forEach(blade => {
      const startX = blade.col * CONFIG.CELL_SIZE + CONFIG.CELL_SIZE / 2;
      const startY = this.grid.getSurfaceRow(blade.col) * CONFIG.CELL_SIZE;
      ...
      if (startX >= minX - 10 && startX <= maxX + 10 && startY >= minY - 15 && startY <= maxY + 15) {
        ...
        ctx.strokeStyle = this.weather === 'Rainy' ? 'hsl(102, 38%, 20%)' : 'hsl(102, 45%, 30%)';
    ...
    this.trees.forEach(tree => {
      ...
      const startY = this.grid.getSurfaceRow(tree.col) * CONFIG.CELL_SIZE;
  ...
  public initializeFoliage() {
    const treeCols = [
      this.grid.nestEntranceCol - 135, // Far-mid left
      this.grid.nestEntranceCol + 45,  // Mid-right
      this.grid.nestEntranceCol + 140  // Far right
    ];
    ...
    treeCols.forEach(col => {
      for (let c = col - 5; c <= col + 5; c++) {
        for (let r = 0; r < CONFIG.ROWS; r++) {
          if (this.grid.isValid(c, r) && this.grid.cells[c][r].type === 'Food') {
            this.grid.cells[c][r].type = 'Sky';
            this.grid.cells[c][r].foodAmount = 0;
          }
        }
      }
    });
  ...
  public triggerFruitDrop() {
    ...
    const treeY = this.grid.getSurfaceRow(target.tree.col) * CONFIG.CELL_SIZE;
    ...
    this.colony.addLog('A ripe fruit detaches from the tree and falls!', 'system');
  }
}
